...
const Invoice = ({ orderInfo, setShowInvoice }) => {
  ...
  const handleSendWhatsApp = () => {
    const url = getWhatsAppReceiptUrl(orderInfo);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center p-4">
      <div className="bg-white p-4 rounded-lg shadow-lg w-full max-w-[400px] max-h-[90vh] overflow-y-auto">
      ...
      <ConfirmDialog
        ...
        message={
          orderInfo?.customerDetails?.phone
            ? `Send this receipt to ${orderInfo.customerDetails.name} at ${orderInfo.customerDetails.phone}.`
            : "Send this receipt over WhatsApp."
        }
        confirmText="Send"
        cancelText="Not now"
        onConfirm={handleConfirmSendWhatsApp}
        onCancel={() => setShowWhatsAppPrompt(false)}
      />
    </div>
  );
};
